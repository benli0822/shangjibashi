...
public class FileServiceImpl implements FileService {
    ...
    @Autowired
    public FileServiceImpl(FileRepository fileRepository, DishRepository dishRepository, MenuRepository menuRepository, ActivityRepository activityRepository) {
    ...
    public boolean addFileToDish(long dish_id, long file_id) {
        ...
        if(!theFile.getDishes().contains(theDish)) {
            ...
            logger.error("neither dish " + theDish.toString() + "nor file " + theFile.toString() + "has already associated with each other more than one time!");
            return false;
        }
    }
    ...
    public boolean addFileToMenu(long menu_id, long file_id) {
        ...
        if(!theFile.getMenus().contains(theMenu)) {
            ...
            logger.error("neither menu " + theMenu.toString() + "nor file " + theFile.toString() + "has already associated with each other more than one time!");
            return false;
        }
    }
    ...
    public boolean addFileToActivity(long activity_id, long file_id) {
        ...
        if(!theFile.getActivities().contains(theActivity)) {
            ...
            logger.error("neither activity " + theActivity.toString() + "nor file " + theFile.toString() + "has already associated with each other more than one time!");
            return false;
        }
    }
