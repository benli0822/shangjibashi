...
public class ActivityServiceImpl implements ActivityService {
    ...
    @Autowired
    public ActivityServiceImpl(ActivityRepository activityRepository, DishRepository dishRepository, ActivityDishRepository activityDishRepository) {
    ...
    public boolean addDishToActivity(long dish_id, long activity_id, int quantity) {
        ...
        logger.debug("Dish " + theDish.toString() + " Activity " + theActivity.toString() + " with quantity: " + quantity);

        // first check the association
        List<ActivityDish> associations = activityDishRepository.findActivityDishByActivityAndDish(theActivity, theDish);
        ...
        if(associations.size() == 0) {
            ...
            logger.error("neither dish " + theDish.toString() + "nor activity " + theActivity.toString() + "has already associated with each other more than one time!");
            return false;
        }
    }
